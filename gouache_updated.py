...
config = configparser.ConfigParser()  # Updated for Python 3
...
def color(source):
    ...
    for y, line in enumerate(lines[2:]):
        for x, item in enumerate(line[4:]):
            try:
                canvas.putpixel((x, y), code[float(item)])
            except:
                continue
...
def cook(save, *data, **opts):
    ...
    if type(cols) != list:
        cols = [cols]
    N = [list(zip(*defs))[0].index(i) + 2 for i in cols]
    ...
    # header
    results = [['read'] + [i for j in cols for i in ['-'] * 4 + [j] + list(window)]]
    ...
    for row in data:
        ...
        E = list(zip(D, [(min(e + [1000]), max(e + [-1])) for e in E]))
        D = [{k: v for k, v in d.items() if e[0] <= k <= e[1]} for d, e in E]
        D = [[i.get(j, '') for j in window] for i in D]

        # optional cutoff for data content
        if all([d.count('') * 100. / len(d) + least > 100 for d in D]):
            continue

        # add to results
        results += [['|'.join(row[:2])] + [i for d in D for i in ['', 10, 10, 10, '', ] + d]]

    # write only if there was data
    if len(results) < 3:
        return
    with open(save, 'w') as handle:
        handle.write('\n'.join(['\t'.join([str(i) for i in j]) for j in results]))
...
def read_from_file(cols, defs, *names):
    ...
    for source in names:
        try:
            MAPit = read_FASTA(source, **dict(defs))
        except:
            continue
